#!/usr/bin/env bash
# Append one master measurement to the perf history repo.
# Needs PERF_DATA_REPO, PERF_DATA_KEY (deploy key with write access),
# SHA and COMMITTED_AT in the environment.
set -ueo pipefail
...
# Another master run may have published while we were measuring.
for _ in 1 2 3; do
    if git push; then
        exit 0
    fi
