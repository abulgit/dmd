...
/// The five Phase A metrics, in the order they appear in the report (plan §6).
immutable MetricDef[] phaseA = [
...
/// Measure every Phase A metric for one dmd binary. `tag` ("base"/"head")
...
/// Byte size of `binary` after copying and stripping it.
...
/// Compile the workload to an executable, strip it, and return its size.
...
/// Peak resident set size (KiB) of compiling the workload, via /usr/bin/time.
